...
describe('User Story 1 - Find and add a book to the list', () => {
  ...
  test('adding a book using the pageCount Open Library reported stores that value as totalPages', async () => {
    const openLibraryClient = createFakeOpenLibraryClient(async () => [searchResult({ pageCount: 320 })]);
    const app = createTestApp({ openLibraryClient });
    ...
    expect(addRes.status).toBe(201);
    expect(addRes.body.totalPages).toBe(320);

    const listRes = await request(app).get('/api/books');
    expect(listRes.body.books[0].totalPages).toBe(320);

    const duplicateRes = await request(app).post('/api/books').send({
      openLibraryWorkId: result.openLibraryWorkId,
      title: result.title,
      author: result.author,
      category: 'want_to_read',
  ...
  test('adding a book via the manual-fallback path still rejects an invalid totalPages', async () => {
    const openLibraryClient = createFakeOpenLibraryClient(async () => [searchResult({ pageCount: null })]);
    const app = createTestApp({ openLibraryClient });

    const searchRes = await request(app).get('/api/search').query({ q: 'book without page data' });
    const result = searchRes.body.results[0];
    ...
    for (const totalPages of [undefined, 0, -5, 1.5]) {
      const res = await request(app).post('/api/books').send({ ...baseRequest, totalPages });
      expect(res.status).toBe(400);
      expect(res.body.error).toMatch(/whole number greater than zero/i);
    }

    const validRes = await request(app).post('/api/books').send({ ...baseRequest, totalPages: 250 });
    expect(validRes.status).toBe(201);
    expect(validRes.body.totalPages).toBe(250);
  });
